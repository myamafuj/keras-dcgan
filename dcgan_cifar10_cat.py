...
def main():
    ...
    train_x = np.concatenate((cifar10_x0, cifar10_x1))
    train_y = np.concatenate((cifar10_y0, cifar10_y1))

    # -1 から 1 に変換
    train_x = train_x[train_y[:, 0] == 3]
    ...
    for epoch in range(epochs):
        ...
        for step in range(steps_per_epoch):
            ...
            # indices = np.random.randint(0, train_size, size=batch_size)
            # real_images = train_x[indices]
        ...
        path = f'figure/dcgan_cifar10_cat/image_{epoch+1:04d}.png'
        if not os.path.exists(os.path.dirname(path)):
            os.makedirs(os.path.dirname(path))
        plt.savefig(path)
        plt.close()

        path = f'var/log/dcgan_cifar10_cat_history.json'
        if not os.path.exists(os.path.dirname(path)):
            os.makedirs(os.path.dirname(path))
