...
def main():
    ...
    p_r = np.random.uniform(7, 12, size=(batch_size, 1)).astype(np.float32)*0.1
    p_f = np.random.uniform(0, 3, size=(batch_size, 1)).astype(np.float32)*0.1

    np.random.seed(0)
    # test_noise = np.random.uniform(-1, 1, size=(30, 100)).astype(np.float32)
    test_noise = np.random.randn(30, 100).astype(np.float32)
    ...
    for epoch in range(epochs):
        ...
        for step in range(steps_per_epoch):
            ...
            #         [real_images, fake_images], [p_f, p_r]
            #     )
            # else:
            #     d_history = train_discriminator.train_on_batch(
        ...
        path = f'figure/dcgan_cat_face_mod/image_{epoch+1:03d}.png'
        ...
        path = f'model/dcgan_cat_face_mod/model_{epoch+1:03d}.h5'
        if not os.path.exists(os.path.dirname(path)):
            os.makedirs(os.path.dirname(path))
        generator.save(path)
